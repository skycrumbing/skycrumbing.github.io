...
**同步与异步**  
同步和异步一般是用来形容一次方法调用。  
同步方法调用一旦开始，调用方必须等到被调用的方法返回结果才能继续后面的行为。而异步是调用方法之后直接返回，调用方继续后面的行为。  
**阻塞和非阻塞**  
非阻塞指在不能立刻得到结果之前，该函数不会阻塞当前线程，而会立刻返回，阻塞相反。  
**临界区**  
...
## ThreadLocal  
线程的私有变量。详见无状态思想[无状态思想](https://skycrumbing.github.io/java/2018/12/15/stateless/ "无状态思想")
