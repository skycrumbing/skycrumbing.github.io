...
private void rotateLeft(Entry<K,V> p) {
    if (p != null) {
         ...
         */
        r.parent = p.parent;
        //如果p为根节点，根节点直接被p的右子节点替代
        if (p.parent == null)
            root = r;
        //如果p为原父节点的左子节点，则右子节点成为原父节点的左子节点
        else if (p.parent.left == p)
            p.parent.left = r;
        //如果p为原父节点的右子节点，则右子节点成为原父节点的左子节点
        else
            p.parent.right = r;
...
```
右旋：将左子节点和被选中节点交换，并进行微调（小的为父，与左旋对称）  
```
...
``` 
## 删除操作
TreeMap是如何删除一个节点的呢，在讲到删除之前先谈谈什么是后继节点。  
### 后继节点  
